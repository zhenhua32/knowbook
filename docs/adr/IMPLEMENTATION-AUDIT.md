...
本文为当时的验收记录。2026-09-12 起，v1 兼容层及对应测试由 [ADR-0008](0008-remove-v1-plugins.md) 移除；下文 v1/Legacy 描述仅为历史，不代表当前支持的运行通道。
...
| 0004 UI 隔离 | ViewSpec v1、11 个 slot、预算/无障碍、opaque 自定义协议 iframe/CSP、受限 MessagePort、隐藏 frame ready 握手、失败不提交、提交后自动恢复、权限化宿主主题切换 | main/renderer UI、frame protocol、activation coordinator、主题插件完整链路与真实 Electron 隔离/主题 E2E |
| 0005 助手事件 | append-only 强类型事件、投影/回放、MiMo-compatible 流式 adapter、多 step agent、工具 guard、精确审批与拒绝防重放、inbox、插件闭环、恢复、事件化自动摘要 | assistant session/agent/tool/plugin-authoring、MiMo 主题插件端到端、renderer conversation、Activity Pulse v2 测试 |
...
- `npm test`：441/441 通过。
- Electron E2E：80/80 场景逐项通过；构建入口检查已改为 `out/main/index.cjs`，不再因错误检查 `.js` 而静默跳过。
