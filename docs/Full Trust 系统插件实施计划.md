...
4. 发布验收缺口：十二类能力的代码入口、后台监督、可跨宿主重连的 service RPC、native probe 和宿主管理的登录启动均已有首轮实现；Windows x64 unpacked 应用已通过真实 SQLite `.node` 加载、插件升级/回滚、C++ 源码针对 Electron ABI 重编译及编译失败保护、卸载清理验收。macOS/Linux 实际打包态 native CI 与源码重编译、三平台真实 Electron ABI 升级、登录启动与真实系统重启、安装包卸载无残留矩阵，以及第 9 节十二项能力的统一验收证据仍待完成。
...
本节记录截至 2026-09-06 的仓库实现快照。“已落地”只表示相应代码及针对性自动化测试已经存在，不等于第 20 节最终验收完成，也不表示 ADR-0007 已达到 `Implemented`。凡缺少打包态、跨平台或完整 E2E 证据的项目，均保留在“仍待完成”列。
...
- 本轮类型检查、生产构建及完整 596 项测试已通过；标准打包入口的两项 native E2E 一起通过（1.8 分钟），包含打包 smoke 对退出码与最终 JSON 结果联合判定的回归：退出码为 0 但结果缺失、损坏、started 或 failed 时不能判成功；超时终止后等待进程关闭再清理隔离 profile。此前开发态的原生插件、Full Trust 完整链路、升级回滚及 v2 iframe 共四项 E2E 已通过；其中离线 npm 用例在沙箱内遇到 EPERM，单独在沙箱外复跑通过。
...
- [三平台 CI](../.github/workflows/ci.yml) 已接入 SQLite 与源码重编译两项 packaged native 专用验收，并配置 Python 3.12：Windows/macOS 直接运行，Linux 经 `xvfb-run --auto-servernum` 运行；当前只取得本机 Windows 的实际打包态结果，macOS/Linux CI 结果仍待执行确认。native spec 在成功或失败的 `finally` 中将 JSON 写入 `test-results/**/native-acceptance-evidence.json` 并作为文件附件保存，内容包括进程平台/架构/Electron/ABI、binary hash、SQLite 数据、完成阶段、失败阶段和清理错误；源码 spec 另写入 `native-rebuild-evidence.json` 和 `native-compiler.log`，记录真实编译 ABI/hash、成功及失败 job、完成阶段和清理错误。CI 以 `native-system-plugin-${{ matrix.os }}` 上传 `test-results/**`。这些是独立 native 专用夹具，不能作为同一个受控验收插件已完成十二项能力的证据。
- 当前 Windows E2E 尚未覆盖 pnpm/yarn、独立生命周期/build 脚本、macOS/Linux native 源码重编译、三平台真实 Electron ABI 升级、剪贴板/菜单/托盘/外部程序、detached/OS 登录启动、真实 login item 或专属 preload；本次 unpacked 验收也不替代 NSIS 安装/卸载或 macOS/Linux 验证。
- 发布前仍必须完成：macOS/Linux 实际打包态 native CI 与源码重编译，三平台真实 Electron ABI 升级与登录启动项测试；真实系统登录/重启、宿主应用升级/回滚和安装包卸载残留验证；第 9 节十二项能力的统一验收矩阵。ADR-0007 继续保持 `Accepted`。
...
截至 2026-09-06，本节尚未全部满足，ADR-0007 保持 `Accepted`，不得改为 `Implemented`。Windows unpacked SQLite 与真实源码重编译验收已通过；当前最明确的阻塞项是 macOS/Linux 实际打包态 native CI 与源码重编译、三平台真实 Electron ABI 升级与 login、真实系统登录/重启/安装包卸载无残留矩阵，以及第 9 节十二项能力的统一验收证据。
