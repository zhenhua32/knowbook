...
if (process.platform !== 'win32') throw new Error('Install interruption acceptance requires Windows.')
...
process.exitCode = await new Promise((resolve, reject) => {
  const child = spawn(process.execPath, [join(root, 'node_modules/@playwright/test/cli.js'), 'test',
    'system-plugins-install-interruption.spec.ts', '--reporter=list', '--output=test-results/install-interruption'
