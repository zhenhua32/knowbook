...
if (process.platform !== 'win32') throw new Error('Activation recovery acceptance currently requires Windows.')
...
process.exitCode = await new Promise((resolve, reject) => {
  const child = spawn(process.execPath, [join(root, 'node_modules/@playwright/test/cli.js'), 'test',
    'system-plugins-activation-recovery.spec.ts', '--reporter=list', '--output=test-results/activation-recovery'],
