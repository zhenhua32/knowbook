...
function createUiText(language: UiLanguage) {
  ...
  return {
    ...
    blockTreeLabel: zh ? '块树' : 'Block tree',
    savedHierarchy: zh ? '已保存层级' : 'Saved hierarchy',
    noBlockRelationships: zh ? '还没有块关系。' : 'No block relationships yet.',
    draftTreeLabel: zh ? '草稿树' : 'Draft tree',
    draftHierarchy: zh ? '草稿层级' : 'Draft hierarchy',
    depthPreview: zh ? '深度预览' : 'depth preview',
    noDraftBlocks: zh ? '还没有草稿块。' : 'No draft blocks yet.',
    depthLabel: (depth: number) => zh ? `深度 ${depth}` : `depth ${depth}`,
