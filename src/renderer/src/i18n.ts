...
function createUiText(language: UiLanguage) {
  ...
  const blockTypeOptions: Record<string, string> = {
    ...
    'heading-3': zh ? '三级标题' : 'Heading 3',
    'heading-4': zh ? '四级标题' : 'Heading 4',
  ...
  return {
    ...
    wideModeShort: zh ? '宽屏' : 'Wide',
    wideModeEnable: zh ? '开启宽屏模式' : 'Enable wide mode',
    wideModeDisable: zh ? '退出宽屏模式' : 'Exit wide mode',
