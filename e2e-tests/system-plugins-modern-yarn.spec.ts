...
const manager = 'yarn' as const
  test('modern Yarn installs and builds a locked local dependency in packaged Electron @modern-yarn', async ({}, testInfo) => {
    test.setTimeout(240_000)
    expect(process.platform).toBe('win32')
    expect(process.env.KNOWBOOK_E2E_EXECUTABLE, 'Run npm run test:packaged-modern-yarn.').toBeTruthy()
    const pluginId = 'system.e2e.modern-yarn'
    ...
    const prepare = async (version: string, allowScripts: boolean) => {
      ...
      writeFileSync(join(source, 'plugin.json'), JSON.stringify({
        ...
        dependencies: { packageManager: manager, yarnMode: 'modern', install: 'ci', allowScripts, rebuildNativeModules: false, buildCommand: [manager, 'run', 'build'] }
      ...
      expect(request.dependencyPlan).toMatchObject({ yarnMode: 'modern' })
    ...
    try {
      cpSync(join(root, 'e2e-tests/fixtures/modern-yarn'), source, { recursive: true })
      ...
      const expectedVersions = JSON.parse(readFileSync(join(root, 'e2e-tests/fixtures/modern-yarn-toolchain/package.json'), 'utf8')).dependencies
      expect(version).toBe(expectedVersions['@yarnpkg/cli-dist'])
      ...
      for (const config of [{ version: '1.0.0', scripts: false }, { version: '2.0.0', scripts: true }]) {
        ...
        expect(jobs[0].command).toEqual([manager, 'install', '--immutable', '--inline-builds', ...(!config.scripts ? ['--mode=skip-build'] : [])])
        expect(jobs[1].command).toEqual([manager, 'run', 'build'])
        expect(logs.at(-1)).toContain('"YARN_NODE_LINKER":"node-modules"')
        ...
        expect(existsSync(join(runtime, '.pnp.cjs'))).toBe(false)
        expect(readFileSync(join(artifact, '.yarnrc.yml'), 'utf8')).toContain('nodeLinker: pnp')
      ...
      expect(logs.at(-1)).toMatch(/YN0028|lockfile would have been modified/i)
      ...
      await boot()
      await expect.poll(state).toMatchObject({ status: 'active', currentVersion: '2.0.0' })
