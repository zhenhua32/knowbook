...
test('import report lists successful files, filters issues, locates saved blocks and survives cancellation @electron', async () => {
  ...
  await withElectronApp(async ({ app, page, tempRoot }) => {
    ...
    writeFileSync('test-results/markdown-import-report.json', JSON.stringify({ importedFiles: 2, expectedIssues: 7, filtering: true, blockNavigation: true, reopen: true, cancellation: true, errors }, null, 2))
  })
})
