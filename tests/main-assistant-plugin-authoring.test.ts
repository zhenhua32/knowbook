...
test('assistant plugin tools define, diff, approve once, and atomically activate a session preview', async () => {
  await withAuthoringPlatform(async (platform) => {
    ...
    appendActivationToolCall(platform, toolCallId, first.revision.id, scope)
    ...
    assert.deepEqual(
      platform.kernel.listContributions<PluginJsonValue>('workspace.dashboard', scope).map((entry) => entry.value),
...
test('assistant plugin approval is bound to persisted tool arguments and cannot promote preview to workspace', async () => {
  await withAuthoringPlatform(async (platform) => {
    ...
    appendActivationToolCall(platform, wrongToolCallId, 'sha256:wrong', platform.sessionScope)
    ...
    appendActivationToolCall(platform, workspaceCallId, defined.revision.id, workspaceScope)
    assert.throws(() => platform.service.requestActivation({
      ...platform.context,
...
test('rejected or expired approvals never create plugin runs or reusable grants', async () => {
  ...
  await withAuthoringPlatform(async (platform) => {
    ...
    appendActivationToolCall(platform, rejectedCall, defined.revision.id, platform.sessionScope)
    ...
    appendActivationToolCall(platform, expiredCall, defined.revision.id, platform.sessionScope)
...
test('failed validation remains inspectable but cannot request activation', async () => {
  ...
  await withAuthoringPlatform(async (platform) => {
    ...
    appendActivationToolCall(platform, toolCallId, defined.revision.id, platform.sessionScope)
    assert.throws(() => platform.service.requestActivation({
      ...platform.context,
      toolCallId
    }, {
...
function appendActivationToolCall(
  ...
  platform.store.assistantSessions.append(platform.sessionId, {
    ...
    payload: {
      ...
      version: 1,
      arguments: { pluginId: 'daily-review', revisionId, scope }
    }
  })
}
