...
test('links are re-synced after target title change', () => {
  withStore((store, backupRoot) => {
    ...
    assert.equal(productAfter.outgoingLinks.some((item) => item.id === roadmap.id && item.path === 'Home/Product/Execution Plan'), true)
    assert.ok(productAfter.blocks.some((block) => block.content.includes('[[Home/Product/Execution Plan]]')))
    assert.ok(store.getDocumentDetail(roadmap.id)!.backlinks.some((item) => item.id === product.id))
